...
int main(int argc, char** argv) {
  ...
  if (!cam_list.empty() && !cam_list[ui_cam_idx].resolutions.empty()) {
    // Smart default: pick reasonable resolution instead of max (to avoid lag)
    const auto& resolutions = cam_list[ui_cam_idx].resolutions;
    auto wh = resolutions[0]; // Start with minimum
    
    // Prefer common resolutions in order: 640x480, 800x600, 1024x768, 1280x720
    for (const auto& res : resolutions) {
      if ((res.first == 640 && res.second == 480) ||
          (res.first == 800 && res.second == 600) ||
          (res.first == 1024 && res.second == 768) ||
          (res.first == 1280 && res.second == 720)) {
        wh = res;
        break;
      }
    }
  ...
  int ui_vcam_idx = 0; segmecam::VCam vcam;
  ...
  while (running) {
          ...
          if (fx_skin_adv) {
            ...
            if (fx_adv_scale < 0.999f) {
              ...
              if (roi.width >= 8 && roi.height >= 8) {
                ...
                int down_interp = (sc >= 0.85f) ? cv::INTER_LINEAR : cv::INTER_AREA;
                cv::Mat small; cv::resize(roi_bgr, small, cv::Size(), sc, sc, down_interp);
                ...
                ApplySkinSmoothingAdvBGR(small, fr_small, fx_skin_amount, fx_skin_radius*sc, fx_skin_tex, fx_skin_edge*sc, lmsp_small,
                                         ...
                                         fx_wrinkle_neg_cap);
                cv::Mat up; cv::resize(small, up, roi.size(), 0, 0, cv::INTER_CUBIC);
                // Optional detail re-injection to counter down/up sampling softness
                float dp = std::clamp(fx_adv_detail_preserve, 0.0f, 0.5f);
                if (dp > 1e-3f) {
                  // Build feathered face mask in ROI
                  ...
                  cv::Mat mask_f; mask_roi_u8.convertTo(mask_f, CV_32F, 1.0/255.0);
                  // Unsharp: add fraction of high-frequency detail from original ROI
                  cv::Mat base; cv::GaussianBlur(roi_bgr, base, cv::Size(0,0), 0.8);
                  cv::Mat hi; cv::Mat roi32, base32; roi_bgr.convertTo(roi32, CV_32F, 1.0/255.0); base.convertTo(base32, CV_32F, 1.0/255.0);
                  hi = roi32 - base32; // -1..1 roughly
                  std::vector<cv::Mat> uch(3), mch(3), och(3); cv::split(up, uch);
                  std::vector<cv::Mat> hi_ch(3); cv::split(hi, hi_ch);
                  for (int i=0;i<3;++i) {
                    cv::Mat u32; uch[i].convertTo(u32, CV_32F, 1.0/255.0);
    ...
    if (vcam.IsOpen() && !display_rgb.empty()) {
      // Ensure format matches; if not, try to reconfigure
      if (display_rgb.cols != vcam.Width() || display_rgb.rows != vcam.Height()) {
        if (ui_vcam_idx >=0 && ui_vcam_idx < (int)vcam_list.size()) vcam.Open(vcam_list[ui_vcam_idx].path, display_rgb.cols, display_rgb.rows);
      }
      cv::Mat disp_bgr; cv::cvtColor(display_rgb, disp_bgr, cv::COLOR_RGB2BGR);
      vcam.WriteBGR(disp_bgr);
    }
        ...
        if (vcam_list.empty()) {
          ...
          if (!vcam.IsOpen()) {
            if (ImGui::Button("Start##vcam")) {
              ...
              if (ui_vcam_idx >=0 && ui_vcam_idx < (int)vcam_list.size()) vcam.Open(vcam_list[ui_vcam_idx].path, W, H);
            }
          } else {
            if (ImGui::Button("Stop##vcam")) { vcam.Close(); }
          }
        }
        ImGui::Separator();
      }
        ...
        if (ImGui::CollapsingHeader("Skin Smoothing")) {
          ...
          if (fx_skin) {
            if (!fx_skin_adv) {
              ...
              if (fx_adv_scale < 0.999f) {
                ImGui::SliderFloat("Detail preserve", &fx_adv_detail_preserve, 0.0f, 0.5f);
              }
